...
import { IServiceSoap12Soap, ServiceSoap12Types } from "@infrastructure/outbound/ports/soap/interfaces/Service/ServiceSoap12";
import { IServiceSoapSoap, ServiceSoapTypes } from "@infrastructure/outbound/ports/soap/interfaces/Service/ServiceSoap";
...
import { VoucherType, ConceptType, DocumentType, CurrencyType, OptionalType, TaxType } from "@domain/types/electronic-billing.types";
import {
    mapServerStatus,
    mapSalesPoints,
    mapLastVoucher,
...
export class ElectronicBillingRepository extends BaseSoapRepository implements IElectronicBillingRepositoryPort {
    ...
    private async getClient(): Promise<IServiceSoapSoap | IServiceSoap12Soap> {
        ...
        const wsdlName = this.production ? WsdlPathEnum.WSFE : WsdlPathEnum.WSFE_TEST;
        const endpoint = this.production ? EndpointsEnum.WSFEV1 : EndpointsEnum.WSFEV1_TEST;

        let client: IServiceSoapSoap | IServiceSoap12Soap;
        let soapVersion: SoapServiceVersion;

        if (this.useSoap12) {
            client = await this.soapClient.createClient<IServiceSoap12Soap>(wsdlName, {
    ...
    async getLastVoucher(salesPoint: number, voucherType: number): Promise<LastVoucherResultDto> {
        const client = await this.getClient();
    ...
    async getIvaReceptorTypes(claseCmp?: string): Promise<IvaReceptorTypesResultDto> {
        const client = await this.getClient();
    ...
    async informCaeaNoMovement(caea: string, salesPoint: number): Promise<CaeaNoMovementResultDto> {
        const client = await this.getClient();
    ...
    async consultCaeaNoMovement(caea: string, salesPoint: number): Promise<CaeaNoMovementResultDto> {
        const client = await this.getClient();
    ...
    async informCaeaUsage(voucher: Voucher, caea: string): Promise<CaeaUsageResultDto> {
        const client = await this.getClient();
        const voucherData = voucher.toDTO();
        ...
        return {
            resultGet: result.FeDetResp?.FECAEADetResponse?.[0] ? mapCaeaUsage(result.FeDetResp.FECAEADetResponse[0] as any) : undefined,
            errors: mapSoapErrors(result.Errors) ? { err: mapSoapErrors(result.Errors)! } : undefined
        };
    }
