...
export class RegisterScopeTenRepository extends BaseSoapRepository implements IRegisterScopeTenRepositoryPort {
    ...
    private async getClient(): Promise<IPersonaServiceA10PortSoap> {
        ...
        const wsdlName = this.production ? WsdlPathEnum.WSSR_PADRON_TEN : WsdlPathEnum.WSSR_PADRON_TEN_TEST;
        const endpoint = this.production ? EndpointsEnum.WSSR_PADRON_TEN : EndpointsEnum.WSSR_PADRON_TEN_TEST;

        const client = await this.soapClient.createClient<IPersonaServiceA10PortSoap>(wsdlName, {
