...
export class RegisterScopeThirteenRepository extends BaseSoapRepository implements IRegisterScopeThirteenRepositoryPort {
    ...
    private async getClient(): Promise<IPersonaServiceA13PortSoap> {
        ...
        const wsdlName = this.production ? WsdlPathEnum.WSSR_PADRON_THIRTEEN : WsdlPathEnum.WSSR_PADRON_THIRTEEN_TEST;
        const endpoint = this.production ? EndpointsEnum.WSSR_PADRON_THIRTEEN : EndpointsEnum.WSSR_PADRON_THIRTEEN_TEST;

        const client = await this.soapClient.createClient<IPersonaServiceA13PortSoap>(wsdlName, {
    ...
    async getTaxpayerDetails(identifier: number): Promise<TaxpayerDetailsDto | null> {
        const client = await this.getClient();
    ...
    async getTaxIDByDocument(documentNumber: string): Promise<TaxIDByDocumentResultDto> {
        const client = await this.getClient();
        ...
        try {
            ...
            const idPersona = Array.isArray(idPersonaListReturn.idPersona) ? idPersonaListReturn.idPersona : typeof idPersonaListReturn.idPersona === "number" ? [idPersonaListReturn.idPersona] : [];
