...
export class RegisterScopeFourRepository extends BaseSoapRepository implements IRegisterScopeFourRepositoryPort {
    ...
    private async getClient(): Promise<IPersonaServiceA4PortSoap> {
        ...
        const wsdlName = this.production ? WsdlPathEnum.WSSR_PADRON_FOUR : WsdlPathEnum.WSSR_PADRON_FOUR_TEST;
        const endpoint = this.production ? EndpointsEnum.WSSR_PADRON_FOUR : EndpointsEnum.WSSR_PADRON_FOUR_TEST;

        const client = await this.soapClient.createClient<IPersonaServiceA4PortSoap>(wsdlName, {
