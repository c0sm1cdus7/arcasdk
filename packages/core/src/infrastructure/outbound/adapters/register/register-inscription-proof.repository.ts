...
export class RegisterInscriptionProofRepository extends BaseSoapRepository implements IRegisterInscriptionProofRepositoryPort {
    ...
    private async getClient(): Promise<IPersonaServiceInscriptionProofPortSoap> {
        ...
        const wsdlName = this.production ? WsdlPathEnum.WSSR_INSCRIPTION_PROOF : WsdlPathEnum.WSSR_INSCRIPTION_PROOF_TEST;
        const endpoint = this.production ? EndpointsEnum.WSSR_INSCRIPTION_PROOF : EndpointsEnum.WSSR_INSCRIPTION_PROOF_TEST;

        const client = await this.soapClient.createClient<IPersonaServiceInscriptionProofPortSoap>(wsdlName, { forceSoap12Headers: false });
    ...
    async getTaxpayersDetails(identifiers: number[]): Promise<TaxpayersDetailsDto> {
        const client = await this.getClient();
        ...
        return {
            persona: personaListReturn.persona?.map((p: any) => this.mapPersonaToDto(p)) || [],
