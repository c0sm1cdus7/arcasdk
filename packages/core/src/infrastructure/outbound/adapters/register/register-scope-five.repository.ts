...
export class RegisterScopeFiveRepository extends BaseSoapRepository implements IRegisterScopeFiveRepositoryPort {
    ...
    private async getClient(): Promise<IPersonaServiceA5PortSoap> {
        ...
        const wsdlName = this.production ? WsdlPathEnum.WSSR_PADRON_FIVE : WsdlPathEnum.WSSR_PADRON_FIVE_TEST;
        const endpoint = this.production ? EndpointsEnum.WSSR_PADRON_FIVE : EndpointsEnum.WSSR_PADRON_FIVE_TEST;

        const client = await this.soapClient.createClient<IPersonaServiceA5PortSoap>(wsdlName, {
